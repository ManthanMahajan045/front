...
import { CalendarDays, ChevronRight, Clock3, MapPin, ThumbsUp, X } from "lucide-react";
import TopBar from "../components/TopBar";
import BottomNav from "../components/BottomNav";
import { getMyReports, auth } from "../firebase";
import { signOut } from "firebase/auth";

const SETTINGS_ROWS = [
  { key: "saved", label: "Saved Location", target: "saved" },
...
export default function ProfileScreen({ onNavigate, onMenu, user }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setReportsError("");
      try {
        const mine = await getMyReports(user.firebaseUid);
        if (active) setReports(mine);
      } catch (error) {
        if (active) {
          setReports([]);
          setReportsError("Your reports could not be loaded. Please try again.");
        }
        console.warn("My reports unavailable:", error?.message || error);
      } finally {
  ...
  return (
    <div className="screen profile-screen">
      ...
      <div className="profile-hero">
        ...
        <div><div className="profile-name">{name}</div><div className="profile-subtitle">RoadSense Member</div></div>
      </div>

      <div className="section-label">My Reports</div>
      <div className="my-reports-list">
        {reportsLoading && <div className="my-reports-state">Loading your reports…</div>}
        ...
        {!reportsLoading && !reportsError && reports.length === 0 && (
          <div className="my-reports-empty">
            ...
            <span>Hazards you report will appear here.</span>
            <button onClick={() => onNavigate("report")}>Report a Hazard</button>
          </div>
        )}
      ...
      <div className="settings-list">
        {SETTINGS_ROWS.map((row) => <button className="list-row" key={row.key} onClick={() => openSetting(row)}><span>{row.label}</span><ChevronRight size={16} className="chevron" /></button>)}
      </div>

      <button className="list-row danger" onClick={() => setModal("logout")}>Logout</button>
      <BottomNav active="profile" onNavigate={onNavigate}/>

      {modal && <div className="profile-modal-backdrop" onClick={() => setModal(null)}>
