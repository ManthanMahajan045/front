...
export default function SavedPlacesScreen({ onNavigate, onSelectLocation, user }) {
  const [places, setPlaces] = useState([]), [status, setStatus] = useState("loading"), [error, setError] = useState(""), [editorOpen, setEditorOpen] = useState(false), [editing, setEditing] = useState(null), [type, setType] = useState("custom"), [name, setName] = useState(""), [selected, setSelected] = useState(null), [query, setQuery] = useState(""), [suggestions, setSuggestions] = useState([]), [searching, setSearching] = useState(false), [pickerPoint, setPickerPoint] = useState(null), [address, setAddress] = useState(""), [saving, setSaving] = useState(false), [locating, setLocating] = useState(false);
  ...
  const hasHome = places.some((place) => place.type === "home"), hasWork = places.some((place) => place.type === "work"), selectedPlace = selected || pickerPoint, mapCenter = selectedPlace?.coordinates || DEFAULT_CENTER, canUseCurrent = typeof navigator !== "undefined" && Boolean(navigator.geolocation), emptyCopy = useMemo(() => status === "ready" && places.length === 0, [status, places.length]);
  const resetEditor = () => { setEditorOpen(false); setEditing(null); setQuery(""); setSuggestions([]); setSelected(null); setPickerPoint(null); setAddress(""); setName(""); setError(""); };
  const openEditor = (place = null, initialType = "custom") => { setEditing(place); setType(place?.type || initialType); setName(place?.name || (initialType === "home" ? "Home" : initialType === "work" ? "Work" : "")); setQuery(""); setSuggestions([]); setSelected(place ? normalizeLocation(place) : null); setPickerPoint(place ? normalizeLocation(place) : null); setAddress(place?.address || ""); setError(""); setEditorOpen(true); };
  ...
  const save = async (event) => {
    ...
    setSaving(true);
    try { const payload = { type, name: cleanName, address: address.trim() || selectedPlace.address || `${coordinates.lat.toFixed(5)}, ${coordinates.lng.toFixed(5)}`, latitude: coordinates.lat, longitude: coordinates.lng }; const saved = editing ? await updateSavedLocation(editing.id, payload) : await createSavedLocation(payload); setPlaces((current) => editing ? current.map((place) => place.id === editing.id ? saved : place) : [...current, saved]); resetEditor(); } catch (err) { setError(err?.message || "Could not save this place."); } finally { setSaving(false); }
  };

  const remove = async (place) => { if (!window.confirm(`Delete ${place.name}?`)) return; try { await deleteSavedLocation(place.id); setPlaces((current) => current.filter((item) => item.id !== place.id)); if (selected?.id === place.id) setSelected(null); } catch (err) { setError(err?.message || "Could not delete this place."); } };
