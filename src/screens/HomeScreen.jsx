...
function OpenDirections({ destination }) {
  const openDirections = () => {
    ...
    window.open(`https://www.google.com/maps/dir/?api=1&destination=${destination.lat},${destination.lng}&travelmode=driving`, "_blank", "noopener,noreferrer");
  };
...
export default function HomeScreen({ onNavigate, selectedLocation, theme, onToggleTheme }) {
  ...
  useEffect(() => {
    ...
    const pollLocation = async () => {
      ...
      try {
        ...
        // Always keep the best fix. Only suppress tiny jitter in the UI.
  ...
  return (
    <div className="screen home-screen">
      ...
      <div className="search-input" onClick={() => onNavigate("search")}>{selectedLocation ? selectedLocation.name : "Search here"}</div>

      <div className="map-wrapper">
        <MapContainer center={[mapCenter.lat, mapCenter.lng]} zoom={14} zoomControl={true} preferCanvas style={{ height: "100%", width: "100%" }}>
          <TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          <RecenterMap center={mapCenter} zoom={selectedLocation ? 16 : 14} />
          {userLocation && !selectedLocation && (
            <>
              <Circle center={[userLocation.lat, userLocation.lng]} radius={Math.max(userLocation.accuracy || 10, 5)} pathOptions={{ color: "#2563eb", fillColor: "#3b82f6", fillOpacity: 0.10, weight: 1 }} />
              <CircleMarker center={[userLocation.lat, userLocation.lng]} radius={7} pathOptions={{ color: "#fff", fillColor: "#2563eb", fillOpacity: 1, weight: 3 }}>
        ...
        <div className="map-actions">
          <button className="map-action-btn" onClick={locateNow} disabled={locating} title="Use my location"><LocateFixed size={15} className={locating ? "spin" : ""} /> {locating ? "Locating…" : "My location"}</button>
      ...
      <div className="alert-banner">
        ...
        <div className="alert-banner-meta">{nearestAlert.distance !== undefined ? `${(nearestAlert.distance * 1000).toFixed(0)} m ahead` : "Nearby"}</div>
