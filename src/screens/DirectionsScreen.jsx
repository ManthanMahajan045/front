...
const livePin = L.divIcon({ className: "", html: "<div class=\"route-live-pin\"><span></span></div>", iconSize: [28, 28], iconAnchor: [14, 14] });
...
function FitRoute({ route, origin, destination, follow }) {
  ...
  useEffect(() => {
    ...
    if (follow && origin) {
      map.setView([origin.lat, origin.lng], Math.max(map.getZoom(), 15), { animate: true, duration: 0.5 });
...
export default function DirectionsScreen({ onNavigate, selectedLocation, userLocation }) {
  ...
  const startLiveNavigation = () => {
    if (!navigator.geolocation) {
      setError("Live GPS is not supported by this browser.");
      return;
    }
    setIsNavigating(true);
    setError("");
    setGpsStatus("Finding your live location…");
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        setOrigin(next);
        setGpsStatus(`Live • ±${Math.round(next.accuracy || 0)} m accuracy`);
      },
      (gpsError) => {
        setGpsStatus("");
        setError(gpsError.code === 1 ? "Location permission is required for live navigation." : "GPS signal is temporarily unavailable.");
      },
    ...
    rerouteRef.current = window.setInterval(async () => {
      const live = latestGpsRef.current;
      if (!live || !destination) return;
      try {
        const freshRoutes = await getHazardAwareRoutes(live, destination, confirmedHazards);
        setRoutes(freshRoutes);
        setSelectedRoute((current) => Math.min(current, Math.max(0, freshRoutes.length - 1)));
      } catch (err) {
        console.warn("Live reroute failed:", err);
      }
    }, 5000);
  };
  ...
  return (
    <div className="screen directions-screen">
      <div className="route-header">
        <button onClick={() => { stopLiveNavigation(); onNavigate("home"); }} aria-label="Back"><ArrowLeft size={20} /></button>
        <div><strong>{isNavigating ? "Live navigation" : "Safe route"}</strong><span>RoadSense navigation</span></div>
        <Navigation size={20} />
      </div>
      ...
      <div className="route-sheet">
        ...
        {route && <>
          ...
          {!isNavigating && <div className="route-options">
          ...
          <button className="btn-primary route-start" onClick={isNavigating ? stopLiveNavigation : startLiveNavigation}>{isNavigating ? <><Square size={16} /> End navigation</> : <><Navigation size={17} /> Start route</>}</button>
          {isNavigating && <div style={{ textAlign: "center", fontSize: 11, marginTop: 8, opacity: .7 }}>GPS location updates continuously • route refreshes every 5 seconds</div>}
        </>}
      </div>
