...
export default function SearchScreen({ onNavigate, onSelectLocation }) {
  ...
  const showingLive = liveResults !== null; const results = showingLive ? liveResults : searchableLocations;
  return <div className="screen"><TopBar variant="back" title="" onBack={() => onNavigate("home")} onNavigate={onNavigate} /><div className="search-input" style={{ marginTop: 12 }}><input autoFocus placeholder="Search here" value={query} onChange={(e) => setQuery(e.target.value)} />{loading && <Loader2 size={16} className="spin" />}</div><div className="section-label">{showingLive ? "Suggestions" : "Recent"}</div>{error && <p style={{ padding: "0 20px", fontSize: 12, color: "#dc2626" }}>{error}</p>}{!loading && results.length === 0 && <p style={{ padding: "0 20px", fontSize: 13, color: "#6b7280" }}>Koi location nahi mili "{query}" ke liye.</p>}{results.map((place) => <button className="card" key={place.id} onClick={() => handlePick(place)} style={{ display: "flex", alignItems: "flex-start", gap: 10, width: "calc(100% - 40px)", textAlign: "left", cursor: "pointer", background: "#fff" }}><MapPin size={16} style={{ marginTop: 2, flexShrink: 0, color: "#6d28d9" }} /><div><div className="card-title" style={{ color: "#1f1f23" }}>{place.name}</div><div className="card-meta" style={{ color: "#8b8b98" }}>{place.address}</div></div></button>)}</div>;
}
